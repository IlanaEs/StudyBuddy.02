...
// Use fileURLToPath (not .pathname) so paths resolve correctly on Windows,
// where URL.pathname yields a leading-slash form like /C:/... that join() then
// mangles into C:\C:\... — see https://nodejs.org/api/url.html#urlpathname
...
const sqlByFile = Object.fromEntries(
  expectedFiles.map((file) => [file, readFileSync(join(migrationsDir, file), 'utf8')]),
);
