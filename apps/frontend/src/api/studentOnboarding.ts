import { apiRequest } from './client';
import type { ApiResponse } from './client';

export type StudentOnboardingResult = {
  student_id: string;
  intake_id: string;
};

export type OAuthSignupResult = {
  user: {
    id: string;
    email: string;
    role: string;
    full_name: string;
    status: string;
  };
};

export interface InitStudentOnboardingInput {
  full_name: string;
  grade_level: string | null;
  sub_level: string;
  subject_name: string;
  learning_goal: string | null;
  location_preference: 'online' | 'frontal' | 'both';
  city: string;
  budget_min: number | null;
  budget_max: number | null;
  preferred_days: string[];
  preferred_time_ranges: string[];
  learning_style: string[];
  soft_preferences: string[];
  child_name?: string;
}

export async function completeOAuthSignup(
  role: 'student' | 'parent',
  fullName: string,
  accessToken: string,
): Promise<ApiResponse<OAuthSignupResult>> {
  return apiRequest<OAuthSignupResult>(
    '/auth/complete-oauth-signup',
    {
      method: 'POST',
      body: JSON.stringify({ role, full_name: fullName }),
    },
    accessToken,
  );
}

export async function initStudentOnboarding(
  input: InitStudentOnboardingInput,
  accessToken: string,
): Promise<ApiResponse<StudentOnboardingResult>> {
  return apiRequest<StudentOnboardingResult>(
    '/api/student-onboarding/init',
    {
      method: 'POST',
      body: JSON.stringify(input),
    },
    accessToken,
  );
}
