...
import { CalendarClock, CalendarPlus, Clock, Download, ExternalLink, FileText, Image as ImageIcon, Link2, NotebookPen, Paperclip, Video } from 'lucide-react';
...
import { syncLessonToCalendar } from '../../../api/lessons';
...
export function NextLessonActionCard({ lesson }: { lesson: DashboardLesson | null }) {
  ...
  // Meet link generated by the on-demand calendar sync (overlays the lesson's own).
  ...
  // The active Meet link: the lesson's own, or one just generated via Sync.
  const meetingLink = lesson.meetingLink ?? syncedLink;

  // Backfill the Google Calendar event + Meet link for this existing lesson. Uses
  // the teacher's calendar-scoped Google token (captured on calendar-connect, not
  // persisted by Supabase); if none is available, connect first and retry on return.
  async function handleSyncCalendar() {
    ...
    if (!providerToken) {
      try {
        // Full-page redirect to Google; on return the captured token lets the
        // teacher click Sync again and it succeeds.
        await initiateCalendarConnect(`${window.location.origin}/teacher/dashboard`);
      } catch {
        setSyncError('לא ניתן לחבר את Google Calendar כעת. נסו שוב.');
      }
      return;
    }

    setSyncing(true);
    const result = await syncLessonToCalendar(lesson.id, accessToken, providerToken);
    setSyncing(false);
    if ('error' in result) {
      setSyncError(result.error ?? 'הסנכרון ליומן נכשל. נסו שוב.');
      return;
    }
    setSyncedLink(result.data.meetingLink);
  }
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {meetingLink ? (
          <a href={meetingLink} target="_blank" rel="noreferrer" style={joinBtn}>
            <Video size={16} /> הצטרפות לשיעור (Join Lesson)
          </a>
        ) : lesson.status === 'scheduled' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8, alignSelf: 'flex-start' }}>
            <button
              type="button"
              onClick={() => void handleSyncCalendar()}
              disabled={syncing}
              style={{ ...joinBtn, marginTop: 0, opacity: syncing ? 0.7 : 1, cursor: syncing ? 'wait' : 'pointer' }}
            >
              <CalendarPlus size={16} /> {syncing ? 'מסנכרן…' : 'סנכרון ל-Google Calendar (Generate Meet link)'}
            </button>
            {syncError && <span style={{ fontSize: 12, color: sb.error }}>{syncError}</span>}
          </div>
        ) : (
